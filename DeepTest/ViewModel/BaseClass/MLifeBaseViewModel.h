...
FOUNDATION_EXTERN NSString *const MViewModelIDKey;
/// 传递导航栏title的key：例如 导航栏的title...
FOUNDATION_EXTERN NSString *const MViewModelTitleKey;
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
FOUNDATION_EXTERN NSString *const MViewModelUtilKey;
/// 传递webView Request的key：例如 webView request...
FOUNDATION_EXTERN NSString *const MViewModelRequestKey;

@protocol MLifeViewModelServices;


@interface MLifeBaseViewModel : NSObject
...
- (instancetype)initWithServices:(id<MLifeViewModelServices>)services params:(NSDictionary *)params;

/// The `services` parameter in `-initWithServices:params:` method.
@property (nonatomic, readonly, strong) id<MLifeViewModelServices> services;
...
/**
 是否取消掉左滑pop到上一层的功能（栈底控制器无效） 默认不取消（NO）
 */
...
/**
 ...
 This method will be execute after the execution of `-initWithParams:` method. But the premise is that you need to inherit `BaseViewModel`.
 */
- (void)initialize;
